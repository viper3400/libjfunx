...
namespace libjfunx.operating
{
        ...
        public class IniFile
        {
            ...
            /// <exception cref="IOExeption"></exception>
            public IniFile(string filename)
                : this()
            {
                if (!File.Exists(filename))
                {
                    File.WriteAllText(filename, ";Datei automatisch erzeugt");
                }
            ...
            public String FileName
            {
                ...
                {
                    // (Change1/1.0.0.5)
                    // Die ursprüngliche Schlaufe hat verhindert, dass eine Datei neu eingelesen wird,
                    // wenn der Name der gleich war. Das verhindert aber auch ein erneutes Auslesen der Settings
                    //if (fileName != value)
                    //{
                        lines.Clear();
                    //(FS#3 / FS#5)
                        using (StreamReader sr = new StreamReader(value, System.Text.Encoding.Default))
